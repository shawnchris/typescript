...
function minTime(n: number, edges: number[][], k: number): number {
  ...
  let low = 1;
  let high = edges[edges.length - 1][2];
  let ans = high;
...
function check (t: number, n: number, edges: number[][], k: number): boolean {
    const uf = new UF(n);
...
export function run() {
    console.log(minTime(2, [[0, 1, 3]], 2)); // Output: 3
    console.log(minTime(3, [[0, 1, 2], [1, 2, 4]], 3)); // Output: 4
    console.log(minTime(3, [[0, 2, 5]], 2)); // Output: 0
}